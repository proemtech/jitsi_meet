package com.proemtech.jitsi_meet
...
import com.proemtech.jitsi_meet.JitsiMeetPlugin.Companion.JITSI_MEET_CLOSE
import com.proemtech.jitsi_meet.JitsiMeetPlugin.Companion.JITSI_MEET_TAG
...
class JitsiMeetPluginActivity : JitsiMeetActivity() {
    ...
    var jitsiLayout: LinearLayout? = null;
    ...
    override fun onPictureInPictureModeChanged(
        ...
        if (isInPictureInPictureMode) {
            ...
            this.jitsiLayout!!.setVisibility(LinearLayout.GONE);
        } else {
            JitsiMeetEventStreamHandler.instance.onPictureInPictureTerminated()
            this.jitsiLayout!!.setVisibility(LinearLayout.VISIBLE);
        }
    ...
    override fun onResume() {
        super.onResume()
        onStopCalled = false
        registerReceiver(myReceiver, IntentFilter(JITSI_MEET_CLOSE))
    }

    override fun onConferenceWillJoin(data: HashMap<String, Any>?) {
        Log.d(JITSI_MEET_TAG, String.format("JitsiMeetPluginActivity.onConferenceWillJoin: %s", data))
    ...
    override fun onConferenceJoined(data: HashMap<String, Any>?) {
        Log.d(JITSI_MEET_TAG, String.format("JitsiMeetPluginActivity.onConferenceJoined: %s", data))
    ...
    override fun onParticipantLeft(data: HashMap<String, Any>?) {
        Log.d(JITSI_MEET_TAG, String.format("JitsiMeetPluginActivity.onParticipantLeft: %s", data))
    ...
    override fun onPostCreate(savedInstanceState: Bundle?) {
        Log.i(JITSI_MEET_TAG, "ABC Post Create");
        super.onPostCreate(savedInstanceState);
//        logContentView(getWindow().getDecorView(), "");
        val view = window.decorView as ViewGroup;
        Log.d(JITSI_MEET_TAG, "ABC " + view.javaClass.canonicalName);
    ...
    fun test() {
        ...
        try {
            ...
            Log.d(JITSI_MEET_TAG, "ABC " + jitsiView.javaClass.canonicalName);
            var ab = jitsiView.getRootReactView(jitsiView);
            Log.d(JITSI_MEET_TAG, "ABC " + ab.javaClass.canonicalName);
            var rootReactView: ReactRootView = ab as ReactRootView;
            Log.d(JITSI_MEET_TAG, "ABC " + rootReactView.javaClass.canonicalName);
            logContentView(rootReactView.rootViewGroup, "");
        } catch (ex: Exception) {
            Log.e(JITSI_MEET_TAG, "ABC Error", ex);
        }
//        var jitsiFragment: Fragment? = getSupportFragmentManager().findFragmentById(R.id.jitsiFragment);
    ...
    fun prepareWhiteboardLayout(layout: LinearLayout) {
        this.jitsiLayout = LinearLayout(this);
        this.jitsiLayout!!.layoutParams = LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT
        );
        this.jitsiLayout!!.setPadding(25, 25, 25, 25)

        this.jitsiLayout!!.gravity = Gravity.LEFT;
        ...
        //logoImage.setImageURI(Uri.parse("https://www.proemtech.in/wp-content/uploads/2021/02/vidyartham@2x_1.png"));
        if (JitsiMeetPluginActivity.classroomLogoId != null) {
            logoImage.setImageResource(JitsiMeetPluginActivity.classroomLogoId!!);
        }
        ...
        if (JitsiMeetPluginActivity.whiteboardUrl != null) {
            btnTag.setTextColor(Color.WHITE);
            btnTag.setOnClickListener {
                JitsiMeetEventStreamHandler.instance.onWhiteboardClicked();
//                Toast.makeText(this, "Whiteboard", Toast.LENGTH_SHORT).show()
        ...
        this.jitsiLayout!!.addView(logoParentlayout);
        this.jitsiLayout!!.addView(btnParentlayout);
        layout.addView(jitsiLayout, 0);
    }
